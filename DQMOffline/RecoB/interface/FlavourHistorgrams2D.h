...
public:
  ...
  FlavourHistograms2D (TString baseNameTitle_ , TString baseNameDescription_ ,
		      ...
		      bool statistics_ , bool update, std::string folder, bool mc,
                      bool createProfile) ;
...
//   void defineArray ( int * dimension , int max , int indexToPlot ) ;
  ...
  inline TH2F * histo_cdusg () const { return theHisto_cdusg->getTH2F() ; }
  inline TH2F * histo_bb () const { return theHisto_bb->getTH2F() ; }
  ...
  TProfile * profile_cdusg () const { return theProfile_cdusg->getTProfile() ; }
...
//   T *   theVariable   ;
  ...
  MonitorElement *theProfile_cdusg ;
...
FlavourHistograms2D<T, G>::FlavourHistograms2D (TString baseNameTitle_ , TString baseNameDescription_ ,
					  int nBinsX_ , double lowerBoundX_ , double upperBoundX_ ,
                                          int nBinsY_ , double lowerBoundY_ , double upperBoundY_ ,
					  bool statistics_ ,
					  bool update, std::string folder, bool mc, bool createProfile) :
  ...
  theStatistics ( statistics_ ) , theMin(-1.), theMax(-1.), mcPlots_(mc), createProfile_(createProfile)
{
  ...
  if (!update) {
    ...
    if (mcPlots_) {  
      ...
      theHisto_cdusg  = (prov.book2D ( theBaseNameTitle + "CDUSG" , theBaseNameDescription + " cdusg-jets" , theNBinsX , theLowerBoundX , theUpperBoundX , theNBinsY, theLowerBoundY, theUpperBoundY)) ;
	...
	theProfile_cdusg  = (prov.bookProfile ( theBaseNameTitle + "_Profile_CDUSG" , theBaseNameDescription + " cdusg-jets" , theNBinsX , theLowerBoundX , theUpperBoundX , theNBinsY, theLowerBoundY, theUpperBoundY)) ;
	...
	theProfile_cdusg  =prov.access(theBaseNameTitle + "_Profile_CDUSG") ;
...
void FlavourHistograms2D<T, G>::settitle(const char* titleX, const char* titleY) {
 ...
 if(theHisto_all) theHisto_all ->setAxisTitle(titleY, 2) ;
    if (mcPlots_) {  
      ...
      if(theHisto_cdusg)theHisto_cdusg->setAxisTitle(titleY, 2) ;
  ...
  if(createProfile_) {
    ...
    if (mcPlots_ == true) {  
      ...
      if(theProfile_cdusg)theProfile_cdusg->setAxisTitle(titleY, 2) ;
...
void FlavourHistograms2D<T, G>::divide ( const FlavourHistograms2D<T, G> & bHD ) const {
  ...
  theHisto_all  ->getTH2F()-> Divide ( theHisto_all->getTH2F()  , bHD.histo_all () , 1.0 , 1.0 , "b" ) ;    
    if (mcPlots_) {  
      ...
      theHisto_cdusg ->getTH2F()-> Divide ( theHisto_cdusg->getTH2F() , bHD.histo_cdusg() , 1.0 , 1.0 , "b" ) ;
...
void FlavourHistograms2D<T, G>::fillVariable ( const int & flavour , const T & varX , const G & varY ) const {
  ...
  switch( flavour ) {
	...
	theProfile_cdusg->Fill(varX, varY);
      }
      return;
    case 2:
    ...
    case 5:
      ...
      return;
    case 9:
      theHisto_bb->Fill( varX, varY );
      if(createProfile_) theProfile_bb->Fill(varX, varY);
      return;
    case 21:
      theHisto_g->Fill( varX, varY );
      theHisto_dusg->Fill( varX, varY );
      theHisto_cdusg->Fill( varX, varY );
      if(createProfile_) {
...
std::vector<TH2F*> FlavourHistograms2D<T, G>::getHistoVector() const
{
  ...
  histoVector.push_back ( theHisto_all->getTH2F() );
    if (mcPlots_) {  
      ...
      histoVector.push_back ( theHisto_cdusg->getTH2F());
...
std::vector<TProfile*> FlavourHistograms2D<T, G>::getProfileVector() const
{
  ...
  if(createProfile_) {
	...
	profileVector.push_back ( theProfile_cdusg->getTProfile());
